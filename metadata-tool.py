...
# Librerias para exit y time.sleep
from os import system, name
import sys, os, time
from PIL import Image # libreria pa abrir imagen
import pickle # pa base de datos

    
# Libreria parte grafica
try:
...
def db():
    ...
    global baseidioma
    #---------------base de dtos
    #1) Crear variables pre definidas
    baseInsExif = "SI"; baseidioma = "SP";
    #2) Carga base de datos o la crea
    try: #Cargar
        ...
        baseInsExif = "NO"
        pickle.dump([baseInsExif, baseidioma], open("conf.neo","wb")) #Grabar configuracion
        ...
        if baseInsExif == "SI":
            if baseidioma == "SP":
                ...
                try:
                    clear()
                    #self.sp_radio.setChecked(False)
...
class Ui_MainWindow(object):
    ...
    def idioma(self):
        ...
        if self.sp_radio.isChecked():
            ...
            escritorio = varT.escrSP #Activa variable escritorio
            self.rutaE()           
            clear()
            ...
            escritorio = varT.escrEN #Activa variable escritorio
            self.rutaE()
            clear()
    ...
    #abre la imagen o archivo
    def abreimg(self):
        #escritorio = str(self.ruta_e.text())
        #folder = varT.home + usr + escritorio
        folder = str(self.ruta_e.text())       
        imagen = str(self.img_name_e.text())
        rutaCompleta = folder + imagen
    ...
    #Exportar Exif (exiftool -h Tux.png > Tux.html)
    def exportar(self):
        #escritorio = str(self.ruta_e.text())
        #folder = varT.home + usr + escritorio
        folder = str(self.ruta_e.text())
    ...
    # Borrar Exif (exiftool -all= image.jpeg)    
    def limpiaExif(self):
        folder = str(self.ruta_e.text())
        imagen = str(self.img_name_e.text())
        rutaCompleta = folder + imagen
    ...
    def setupUi(self, MainWindow):    
        ...
        self.abrir_img_b.clicked.connect(self.abreimg) # vincula boton radio con funcion
        ...
        self.ver_img_b.clicked.connect(self.cambia_img) # vincula boton radio con funcion
        ...
        self.ver_exif_b.clicked.connect(self.ver_exif) # vincula boton radio con funcion
        ...
        self.exportar_b.clicked.connect(self.exportar) # vincula boton radio con funcion
        ...
        self.borrar_exif_b.clicked.connect(self.limpiaExif) # vincula boton radio con funcion
        ...
        self.si_radio.clicked.connect(self.instalar) # vincula boton radio con funcion 
        ...
        self.no_radio.clicked.connect(self.instalar) # vincula boton radio con funcion 
        ...
        self.sp_radio.clicked.connect(self.idioma) # vincula boton radio con funcion         
        ...
        self.en_radio.clicked.connect(self.idioma) # vincula boton radio con funcion 
        ...
        #llama a la Traduccion x defecto
        if baseidioma == "SP":
            self.retranslate_SP(MainWindow)  
        else:
            self.retranslate_EN(MainWindow)  
